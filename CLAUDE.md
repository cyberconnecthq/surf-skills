...
This is a Claude Code skills repository for Surf platform development. Skills are specialized knowledge modules organized by category (`odin-dev-*` for development, `odin-data-*` for data/analytics, `odin-op-*` for operations).

## Repository Structure

```
odin-skills/
├── odin-dev-golang/              # Go development skill
│   ├── SKILL.md                  # Skill definition with YAML frontmatter
│   ├── learnings.md              # Team-shared learnings (updated by Claude)
│   └── references/               # Detailed reference documentation
├── odin-dev-repo-map/            # Surf platform repo map (~41 active repos)
│   ├── SKILL.md
│   ├── references/
│   └── scripts/
├── odin-dev-dagster/              # Dagster pipeline management skill
│   ├── SKILL.md
│   └── scripts/
...
│   ├── references/
│   └── scripts/
├── odin-data-db/           # Database debugging skill
│   ├── SKILL.md
│   ├── references/
│   │   ├── setup.md
...
│   ├── references/
│   └── fetch_trace.py
├── odin-op-dev-progress/         # Daily team progress report skill
│   ├── SKILL.md
│   ├── references/
│   └── scripts/
└── README.md
...
Steps:
1. Create a directory with category prefix: `odin-dev-<name>/`, `odin-data-<name>/`, or `odin-op-<name>/`
...
2. **Persist the learning**:
   ...
   - Data/schema discoveries (table gotchas, column value enums, query patterns) → Update the relevant skill's `references/` docs (e.g., `analytics-tables.md`, `surf-tables.md`). Keep both `odin-data-clickhouse` and `odin-data-db` in sync since they query the same underlying data.
...
Install via CLI:
```bash
npx skills add cyberconnecthq/odin-skills
```
